...
export class DartExtractor {
    ...
    extractCallGraph(rootNode) {
        ...
        const walk = (node) => {
            ...
            if (node.type === "arguments" && node.previousSibling) {
                ...
                if ((callee.type === "identifier" || callee.type === "qualified") &&
    ...
    extractExtension(node, classes) {
        // Extensions may be named (`extension MyExt on Foo {}`) or unnamed.
        const nameNode = node.childForFieldName("name") ?? findChild(node, "identifier");
        const onNode = node.childForFieldName("on") ??
     ...
     */
    walkClassBody(body, methods, properties) {
        const visit = (node) => {
            switch (node.type) {
     ...
     */
    extractTopLevelFunction(sigNode, rootNode, sigIndex, functions, exports) {
    ...
    extractTopLevelAccessor(node, functions, exports) {
        const name = this.findSignatureName(node);
        if (!name)
            return;
        functions.push({
    ...
    extractParamsFromSignature(sigNode) {
        const params = [];
        const formalParams = sigNode.childForFieldName("parameters") ??
     ...
     */
    extractImportOrExport(node, imports) {
     ...
     */
    addExportFromNamed(node, exports) {
        const nameNode = node.childForFieldName("name") ?? findChild(node, "identifier");
        if (!nameNode)
            return;
